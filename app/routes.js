module.exports = function(app, session, passport, generator, con) {
    ...
    // Ce n'est pas la route par 'défaut' mais une route comme une autre accessible via l'uri 'localhost:3000/'
    //
    //
...
=======
    // L'API c'est toujours côté serveur, donc ici on est dans l'API, les routes ce sont des éléments d'une API, ce serait plutôt 
    // "Récupération des données depuis Angular et traitement de celles-ci dans la route ..."*
    //
    //
    // Récuperation des données de l'API traiter par la page gestionUser.html et  test dans la base si il existe sinon inscription et generation de mot de passe aléatoirement
    app.post('/createUser', function(req, res) {
